...
const Practice = ({ currentLang }) => {
  useTitle('KanJi5 | Practice');
  const { t } = useTranslation();

  const [modalVisibility, setModalVisibility] = useState(false);
  const [practiceType, setPracticeType] = useState('nonfavorites'); // all, favorites, nonfavorites
  const [visiblePracticeData, setVisiblePracticeData] = useState('kanji'); // vocabulary, meaning
  ...
  const initializePracticeSetting = () => {
    ...
    setVisiblePracticeData('kanji');
    localStorage.setItem('visiblePracticeData', 'kanji');
  };
